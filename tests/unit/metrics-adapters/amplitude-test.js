...
module('amplitude adapter', function(hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    adapter = this.owner.factoryFor('ember-metrics@metrics-adapter:amplitude').create({ config });
  });
  ...
  test('#identify sets the distinct user ID, and calls amplitude with any extra user properties', function(assert) {
    ...
    assert.ok(instanceStub.identify.calledWith(identityStub), 'it sends the correct user identity');
